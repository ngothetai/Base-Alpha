...
class FutureDataset(StockDataset):
    def __init__(self, stock_csv_path, exp_path, **kwargs) -> None:
        ...
        self._split_methods = {
            ...
            'walk_forward': self._split_by_walk_forward
        }
    ...
    def _split_by_sequence(self, **kwargs) -> Tuple[Tuple[pd.DataFrame, ...], ...]:
        try:
            split_date = kwargs['split_date']
        except KeyError:
            raise ValueError("Please provide a split date")
        self._train_data: pd.DataFrame = self._stock_data.loc[self._stock_data.index < split_date]
        self._test_data: pd.DataFrame = self._stock_data.loc[self._stock_data.index >= split_date]
        return tuple([tuple([self._train_data, self._test_data])])
    
    def _split_by_walk_forward(self, **kwargs) -> Tuple[Tuple[pd.DataFrame, ...], ...]:
        try:
            splitter = TimeSeriesSplit(**kwargs)
            res = splitter.split(self._stock_data)
            return tuple(res)
        except KeyError:
    ...
    def _split_train_test_data(self, method: str, **kwargs) -> Tuple[Tuple[pd.DataFrame, ...], ...]:
        try:
            return self._split_methods[method](**kwargs)
        except KeyError:
