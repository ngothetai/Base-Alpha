...
def maximum_drawdown_future(gain, cash_max):
    ...
    return round((gain.cumsum().cummax() - gain.cumsum()).max(), 2), round((gain.cumsum().cummax() - gain.cumsum()).max()/cash_max*100, 2)
...
def Margin(test):
    ...
    return test, round(test.total_gain.iloc[-1]/(test.inLong * test.Close + test.inShort * test.Close + test.outLong * test.Close + test.outShort * test.Close).sum()*10000, 2)
...
class BacktestInformation:
    ...
    def __init__(self, Datetime, Position, Close, fee=0.4):
        ...
        self.fee = fee + 0.3*self.Trading_per_day()
    ...
    def Profit_per_trade(self):
        ...
        return self.Plot_PNL(plot=False)['total_gain_after_fee'].iloc[-1]/self.Number_of_trade()
    ...
    def Return(self):
        ...
        cash_max = self.PNL()[1]
        return round(self.Profit_after_fee()/(len(self.PNL()[0].resample("1D").last()) / 365)/cash_max, 2)  
    ...
    def Plot_PNL(self, window_MA=None, plot=True) -> pd.DataFrame | Tuple[pd.DataFrame, Dict[str, float]]:
        ...
        total_gain.set_index('Datetime', inplace=True)
        self.df.set_index('Datetime', inplace=True)
        ...
        if plot:
            ...
            data = [('Total trading quantity', round(self.Number_of_trade(), 2)),
                    ...
                    ('Profit after fee', round(self.Profit_after_fee(), 2)),
                    ('Trading quantity per day', round(self.Number_of_trade()/len(total_gain.total_gain.resample("1D").last().dropna()), 2)),
                    ('Profit per day after fee', round(self.Profit_per_day(), 2)),
                    ('Return', round(self.Return(), 2)),
                    ('Profit per year', round(self.Profit_per_year(), 2)),
                    ('HitRate', round(self.Hitrate(), 2)),
                    ('HitRate per day', round(self.Hitrate_per_day()[1], 2)),
                    ]
            for row in data:
